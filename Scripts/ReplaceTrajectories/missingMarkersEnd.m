function [missing] = missingMarkersEnd(position)
% Inputs: position: marker position struct
% Outputs: List of markers that are missing from the end of the trial
...
for i = 1:length(fields)
   ...
   if position.(fields{i})(end,4) == 0 % No data exists
       missing{count,1} = fields{i};
       count = count + 1;
       disp(strcat("No data for marker ",fields{i}))
   elseif position.(fields{i})(end,4) == 1
